...
class VencordNative(private val activity: MainActivity, private val wv: WebView) {
    ...
    fun updateVencord() {
        ...
        val conn = HttpClient.fetch(sPrefs.getString("vencordLocation", if(sPrefs.getBoolean("equicord", false)) Constants.EQUICORD_BUNDLE_URL else Constants.VENCORD_BUNDLE_URL)!!)
